...
<body>
    <div class="header">
        ...
        <div class="controls">
            ...
            <button onclick="toggleAutoRefresh()" id="autoRefreshBtn">⏸️ 暂停</button>
            <button onclick="toggleAutoRequeue()" id="autoRequeueBtn">♻️ 自动重排: 关闭</button>
        </div>
    </div>
    ...
    <div class="color-key">
        ...
        <div class="color-key-grid">
            ...
            <div class="color-key-item">
                ...
                <span class="color-key-label"><strong>已完成</strong> - 任务成功完成</span>
            </div>
            <div class="color-key-item">
            ...
            <div class="color-key-item">
                ...
                <span class="color-key-label"><strong>已取消</strong> - 手动取消的任务</span>
            </div>
            <div class="color-key-item">
                <div class="color-key-swatch" style="border-left: 3px solid var(--accent-blue); background: transparent;"></div>
                <span class="color-key-label"><strong>工具调用</strong> - 智能体调用工具/函数</span>
            </div>
            <div class="color-key-item">
            ...
            <div class="color-key-item">
                ...
                <span class="color-key-label"><strong>结果</strong> - 工具结果已接收</span>
            </div>
        </div>
    </div>
    ...
    <script>
        ...
        async function fetchSubagents() {
            try {
                ...
                if (!response.ok) {
                    console.warn('智能体 API 返回', response.status);
                    return [];
                }
        ...
        function getStalledReason(agent, isStalled) {
            ...
            if (agent.outcomeStatus && agent.outcomeStatus !== 'ok') {
                reason = `<div style="font-size: 11px; color: #ff9800; margin-top: 8px; padding: 6px; background: rgba(255, 152, 0, 0.1); border-radius: 4px; border-left: 2px solid #ff9800;">⚠️ 错误: ${agent.outcomeStatus || '失败'}${agent.outcomeError ? ' - ' + agent.outcomeError : ''}</div>`;
        ...
        function getConsoleText(agent) {
            ...
            if (lane === 'blocked') {
                if (agent.outcomeError) return `\u5361\u6b7b: ${agent.outcomeError}`;
                if (agent.outcomeStatus && agent.outcomeStatus !== 'ok') return `\u5361\u6b7b: outcome=${agent.outcomeStatus}`;
                return `\u5361\u6b7b: \u957f\u65f6\u95f4\u672a\u66f4\u65b0. ${taskPart}`;
            }
            ...
            return `
                <div class="${cardClass}" id="agent-${agent.sessionId}" data-role="${agent.role || 'subagent'}">
                    <div class="agent-header">
                        ...
                        <span class="status-indicator ${lane === 'completed' ? 'active' : lane === 'in_progress' ? 'working' : lane === 'queue' ? 'idle' : lane === 'blocked' ? 'stalled' : 'stalled'}" title="${lane === 'in_progress' ? '进行中' : lane === 'queue' ? '队列' : lane === 'blocked' ? '阻塞' : lane === 'completed' ? '已完成' : '已取消'}"></span>
                    </div>
        ...
        function renderTranscriptEvent(event) {
            ...
            if (type === 'message') {
                ...
                try {
                    ...
                    display = `<strong>事件 (${type}):</strong> [无法显示]`;
                }
            }
        ...
        function toggleAutoRefresh() {
            ...
            btn.textContent = autoRefresh ? '⏸️ 暂停' : '▶️ 继续';
        ...
        function updateAutoRequeueButton() {
            ...
            if (!btn) return;
            btn.textContent = autoRequeue ? '♻️ 自动重排: 开启' : '♻️ 自动重排: 关闭';
                ...
                board.innerHTML = Object.entries(columns).map(([key, col]) => {
                    ...
                    const tasks = col.agents.map((agent, idx) => {
                        ...
                        const resumeButton = isBlocked && !agent.completed ? 
                            `<button class="resume-button btn-resume" id="resume-${agent.sessionId}" onclick="event.stopPropagation(); resumeAgent('${agent.sessionId}', '', event);">▶️ 恢复</button>` :
                            `<button class="resume-button btn-resume" id="resume-${agent.sessionId}" onclick="event.stopPropagation(); resumeAgent('${agent.sessionId}', '', event);">▶️ 恢复</button>`;
                        const restartButton = `<button class="resume-button btn-restart" onclick="event.stopPropagation(); restartAgent('${agent.sessionId}')">${isBlocked ? '↩ 重新排队' : '🔄 重启'}</button>`;
                        const killButton = `<button class="resume-button btn-kill" onclick="event.stopPropagation(); killAgent('${agent.sessionId}')">⛔ 取消任务</button>`;
                            ...
                            <div class="kanban-task ${movingClass} ${enterClass}" data-session-id="${agent.sessionId}" onclick="${onClickHandler}">
                                <div class="kanban-task-summary">
                                    <div class="kanban-task-header">
                                        <div>
                                            <div class="kanban-task-title" title="${escapeHtml((agent.task || '无任务描述').substring(0, 500))}">${escapeHtml(agent.task || '无任务描述')} ${agent.role === 'main' ? '<span class="role-badge main">主控</span>' : agent.role === 'cron' ? '<span class="role-badge cron">定时</span>' : ''}</div>
...
def _overstory_agents_to_dashboard_format(agents_list):
    ...
    for a in agents_list:
        ...
        out.append({
            ...
            "outcomeStatus": "\u72b6\u6001" if is_terminal else None,
            "displayName": name,
        })
    return out
...
def _enrich_agents_with_runs(agents):
    ...
    for agent in agents:
        ...
        # 项目/上下文: OpenClaw 智能体使用工作区名称; Overstory 智能体保留其项目名从 _overstory_agents_to_dashboard_format
